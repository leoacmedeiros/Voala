...
public class BasePage {
	...
	//passar variavel valor com "-"
	public static void scrollPageUP(int valor) {
		JavascriptExecutor jse = (JavascriptExecutor) getDriver();
		jse.executeScript("window.scrollBy(0, "+ valor +")");
	}

	public static void scrollPageDOWN(int valor) {
		JavascriptExecutor jse = (JavascriptExecutor) getDriver();
		jse.executeScript("window.scrollBy(0, "+ valor +")");
	}
	...
	public static String gerarEmailRandomico() {
		...
		return email + "@automacaoteste.com.br";
	}
	...
	public static void esperarPresencaDoLoading() {
		try {
			...
			while (--i > 0 && aguarde) {
				...
				List<WebElement> lista = getDriver().findElements(By.xpath("//div[@class='AnimatedLoading']"));
	...
	public static void esperarPresencaDoLoadingLogin() {
		try {
			// System.out.println("Loading...");
			int i = 300;
			boolean aguarde = true;
			while (--i > 0 && aguarde) {
				setaTempoDeEsperaDriver(TEMPO_ESPERA_LOADING);
				sleep(1);
				List<WebElement> lista = getDriver().findElements(By.xpath("//div[@id='LoaderAnimation']"));
				// System.out.println("Loading size: " + lista.size());
				setaTempoDeEsperaDriver(TEMPO_ESPERA_DEFAULT);
				aguarde = hasVisibleElementAtList(lista);
